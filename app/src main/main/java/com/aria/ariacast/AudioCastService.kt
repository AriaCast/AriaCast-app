...
class AudioCastService : Service() {
    ...
    private var activeVideoPlugin: IAriaPlugin? = null
    ...
    private fun startCasting(mediaProjectionToken: Intent, serverHost: String, serverPort: Int) {
        ...
        // Handle Video Plugins
        val videoPlugins = pluginManager.getEnabledPlugins().filter { it.hasVideo }
        if (videoPlugins.isNotEmpty()) {
            val plugin = videoPlugins.first()
            pluginManager.bindToPlugin(plugin.packageName, plugin.className) { binder ->
                activeVideoPlugin = IAriaPlugin.Stub.asInterface(binder)
                try {
                    // Video port is 12890 by default in server
                    activeVideoPlugin?.onSessionStartedWithToken(serverHost, 12890, mediaProjectionToken)
                    Log.i(TAG, "Successfully started video session with plugin: ${plugin.name}")
                } catch (e: Exception) {
                    Log.e(TAG, "Failed to start video session with plugin", e)
                }
            }
        }
    ...
    private fun stopCasting() {
        ...
        activeVideoPlugin?.onSessionEnded()
        activeVideoPlugin = null
