...
internal object PluginClassShutter : ClassShutter {
    override fun visibleToScripts(fullClassName: String): Boolean {
        ...
        // 3. Allow Java dynamic proxies generated by Dalvik/ART/JVM for single-abstract-method (SAM)
        // interface adapters (e.g. View.OnClickListener, Runnable callbacks).
        if (isDynamicProxyClass(fullClassName)) {
            return true
        }
                ...
                val uiHelper = object {
                    fun run(f: Runnable) = activity.runOnUiThread {
                        ...
                        try {
                            ...
                            Log.e("PluginUI", "Error in ui.run callback for plugin: ${plugin.name}", e)
                        } finally {
                            RhinoContext.exit()
                        }
                    }
                    ...
                    fun toast(msg: Any?) = toast(msg, Toast.LENGTH_SHORT)
                    fun toast(msg: Any?, duration: Int) = activity.runOnUiThread {
                        try {
                            Toast.makeText(activity, msg?.toString() ?: "", duration).show()
                        } catch (e: Throwable) {
                            Log.e("PluginUI", "Error displaying toast", e)
                        }
                    }
                    fun showInputDialog(
                        title: String,
                            ...
                            activity.audioCastServiceFlow.filterNotNull().collectLatest { s ->
                                s.state.collectLatest { state ->
                                    ...
                                    try {
                                        ...
                                        Log.e("PluginEvents", "Error in onStateChanged for plugin: ${plugin.name}", e)
                                    } finally { RhinoContext.exit() }
                                }
                            }
                        }
                            ...
                            activity.audioCastServiceFlow.filterNotNull().collectLatest { s ->
                                s.audioBufferFlow.collectLatest { buffer ->
                                    ...
                                    try {
                                        ...
                                        Log.e("PluginEvents", "Error in onAudioBuffer for plugin: ${plugin.name}", e)
                                    } finally { RhinoContext.exit() }
                                }
                            }
                        }
                    ...
                    fun onConfigRequested(callback: org.mozilla.javascript.Function) {
                        ...
                        activity.runOnUiThread {
                            ...
                            try {
                                ...
                                Log.e("PluginEvents", "Error in onConfigRequested for plugin: ${plugin.name}", e)
                            } finally { RhinoContext.exit() }
                        }
                    }
                }
